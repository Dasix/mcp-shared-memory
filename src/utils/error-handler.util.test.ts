import { ErrorCode, detectErrorType, buildErrorContext } from "./error-handler.util.js";
...
describe("error-handler.util", () => {
  describe("detectErrorType", () => {
    ...
    it("should detect ip-api.com specific errors from message", () => {
      ...
      const { code: privateCode, statusCode: privateStatus } = detectErrorType(privateIpError);
      ...
      const { code: reservedCode, statusCode: reservedStatus } = detectErrorType(reservedIpError);
    ...
    it("should default to unexpected error when no patterns match", () => {
      const { code, statusCode } = detectErrorType(new Error("some random error"));
  ...
  describe("buildErrorContext", () => {
    it("should build a context object with all parameters", () => {
      const context = buildErrorContext("User", "create", "controllers/user.controller.ts@create", "user123", {
    ...
    it("should build a context object with only required parameters", () => {
      const context = buildErrorContext("User", "list", "controllers/user.controller.ts@list");
    ...
    it("should handle object entityId", () => {
      const context = buildErrorContext("Document", "get", "controllers/document.controller.ts@get", {
        project: "project1",
        id: "doc123",
      });
