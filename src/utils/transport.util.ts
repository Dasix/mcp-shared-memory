...
import { createApiError, createAuthInvalidError, createUnexpectedError, McpError } from "./error.util.js";
...
export function getIpApiCredentials(): IpApiCredentials {
  const methodLogger = Logger.forContext("utils/transport.util.ts", "getIpApiCredentials");
...
export async function fetchIpApi<T>(
  ...
  const methodLogger = Logger.forContext("utils/transport.util.ts", "fetchIpApi");
...
export async function fetchApi<T>(url: string, options: RequestOptions = {}): Promise<T> {
  ...
  try {
    ...
    methodLogger.debug(`API call completed in ${duration}ms with status: ${response.status} ${response.statusText}`, {
      url,
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw createAuthInvalidError("Authentication failed. Check API token if required.");
      } else if (response.status === 403) {
        // Use createAuthInvalidError or a more specific permission error if needed
        throw createAuthInvalidError("Permission denied for the requested resource.");
      } else if (response.status === 404) {
        throw createApiError("Resource not found at the specified URL.", response.status, errorText);
      } else {
