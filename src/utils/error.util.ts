 ...
 */
export function createAuthMissingError(message: string = "Authentication credentials are missing"): McpError {
 ...
 */
export function createAuthInvalidError(message: string = "Authentication credentials are invalid"): McpError {
 ...
 * Create an API error
 */
export function createApiError(message: string, statusCode?: number, originalError?: unknown): McpError {
...
export function getDeepOriginalError(error: unknown): unknown {
  ...
  while (depth < maxDepth && current instanceof Error && "originalError" in current && current.originalError) {
...
export function formatErrorForMcpTool(error: unknown): {
  ...
  const methodLogger = Logger.forContext("utils/error.util.ts", "formatErrorForMcpTool");
  ...
  const errorDetails = originalError instanceof Error ? { message: originalError.message } : originalError;
...
export function formatErrorForMcpResource(
  ...
  const methodLogger = Logger.forContext("utils/error.util.ts", "formatErrorForMcpResource");
  const mcpError = ensureMcpError(error);
  methodLogger.error(`${mcpError.type} error`, mcpError);
...
export function handleCliError(error: unknown): never {
  const methodLogger = Logger.forContext("utils/error.util.ts", "handleCliError");
  const mcpError = ensureMcpError(error);
  methodLogger.error(`${mcpError.type} error`, mcpError);
