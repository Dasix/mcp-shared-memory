 ...
 */
function extractEssentialValues(obj: Record<string, unknown>, keys: string[]): Record<string, unknown> {
...
function formatSourcePath(filePath: string, functionName?: string): string {
  ...
  return functionName ? `[${normalizedPath}@${functionName}]` : `[${normalizedPath}]`;
}
...
class Logger {
   ...
   */
  debugResponse(message: string, response: Record<string, unknown>, essentialKeys: string[]) {
