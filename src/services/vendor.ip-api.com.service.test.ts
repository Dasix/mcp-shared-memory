...
function skipIfRateLimited(error: unknown): void {
  ...
  if (
    ...
    console.warn("Skipping test in CI due to rate limiting:", error.message);
  ...
  describe("get", () => {
    ...
    it("should properly handle private IP address errors", async () => {
      try {
        ...
        await expect(ipApiService.get("192.168.1.1")).rejects.toThrow(McpError);
        ...
        try {
          ...
          expect((error as McpError).message).toContain("Private IP addresses are not supported");
        }
      } catch (error) {
        skipIfRateLimited(error);
      }
    ...
    it("should properly handle reserved range IP address errors", async () => {
      try {
        // Call the service and expect it to throw
        await expect(ipApiService.get("127.0.0.1")).rejects.toThrow(McpError);
        ...
        try {
          ...
          expect((error as McpError).type).toBe(ErrorType.API_ERROR);
          expect((error as McpError).statusCode).toBe(400);
          expect((error as McpError).message).toContain("Reserved IP addresses are not supported");
        }
      } catch (error) {
        skipIfRateLimited(error);
      }
    ...
    it("should reject invalid IP formats", async () => {
      try {
        ...
        await expect(ipApiService.get("invalid-ip")).rejects.toThrow(McpError);
    ...
    it("should handle network errors gracefully", async () => {
      try {
        ...
        await expect(ipApiService.get("0.0.0.0", { useHttps: true })).rejects.toThrow();
      } catch (error) {
        ...
        if (process.env.CI && error instanceof McpError && error.type === ErrorType.API_ERROR) {
          console.warn("Skipping network error test in CI due to environment differences");
          return;
        }
        skipIfRateLimited(error);
      }
