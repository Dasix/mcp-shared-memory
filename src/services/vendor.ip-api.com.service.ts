...
import { IPDetail, IPDetailSchema, IPApiRequestOptions } from "./vendor.ip-api.com.types.js";
...
const serviceLogger = Logger.forContext("services/vendor.ip-api.com.service.ts");
 ...
 */
async function get(ipAddress?: string, options: IPApiRequestOptions = {}): Promise<IPDetail> {
  const methodLogger = Logger.forContext("services/vendor.ip-api.com.service.ts", "get");
  ...
  try {
    ...
    if (rawData.status !== "success") {
      ...
      if (rawData.message) {
        if (rawData.message.includes("private range")) {
          throw createApiError(`Private IP addresses are not supported: ${rawData.message}`, 400, rawData);
        } else if (rawData.message.includes("reserved range")) {
          throw createApiError(`Reserved IP addresses are not supported: ${rawData.message}`, 400, rawData);
        }
      }
    ...
    throw createUnexpectedError("Unexpected service error while fetching IP data", error);
  }
}
