...
const GetIpDetailsToolSchema = z.object({
  ipAddress: z.string().optional().describe("IP address to lookup (omit for current IP)"),
...
async function handleGetIpDetails(args: GetIpDetailsToolArgsType) {
  const methodLogger = Logger.forContext("tools/ipaddress.tool.ts", "handleGetIpDetails");
...
function registerTools(server: McpServer) {
  const methodLogger = Logger.forContext("tools/ipaddress.tool.ts", "registerTools");
