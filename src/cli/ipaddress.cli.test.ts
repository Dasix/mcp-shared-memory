...
describe("IP Address CLI Commands", () => {
  ...
  beforeAll(() => {
    ...
    if (!credentials.apiToken) {
      console.warn("WARNING: No IP API token available. Tests will run with free tier API access.");
    }
  });
  ...
  describe("get-ip-details command", () => {
    ...
    it("should retrieve details for a specific valid IP (Google DNS)", async () => {
      const result = await CliTestUtil.runCommand(["get-ip-details", "8.8.8.8"]);
    ...
    it("should handle invalid IP format gracefully", async () => {
      const result = await CliTestUtil.runCommand(["get-ip-details", "invalid-ip-format"]);
    ...
    it("should handle private/reserved IP addresses correctly", async () => {
      const result = await CliTestUtil.runCommand(["get-ip-details", "192.168.1.1"]);
      ...
      if (result.exitCode === 0) {
        ...
        expect(result.stderr).toMatch(/private|reserved|invalid|error/i);
      }
    }, 15000);

    it("should handle help flag correctly", async () => {
      const result = await CliTestUtil.runCommand(["get-ip-details", "--help"]);
    ...
    it("should handle unknown flags gracefully", async () => {
      const result = await CliTestUtil.runCommand(["get-ip-details", "--unknown-flag"]);
