...
const versionFiles = [
  {
    ...
    pattern: /"version": "([^"]*)"/,
    replacement: (match, currentVersion) => match.replace(currentVersion, newVersion),
  },
  {
...
function updateFileVersion(fileConfig) {
  const { path: filePath, pattern, replacement, optional = false } = fileConfig;
  ...
  try {
    ...
    if (currentVersion === newVersion) {
      log(`Version in ${path.basename(filePath)} is already ${newVersion}`, true);
      return;
    }
    ...
    if (options.dryRun) {
      log(`Would update version in ${filePath} from ${currentVersion} to ${newVersion}`);
    } else {
      ...
      log(`Updated version in ${path.basename(filePath)} from ${currentVersion} to ${newVersion}`);
    }
  } catch (error) {
    if (optional) {
...
try {
  ...
  if (!newVersion) {
    ...
    log(`No version specified, using version from package.json: ${newVersion}`);
  }
